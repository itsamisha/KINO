  ...
  try {
    ...
    if (userInfo.user_type === "customer") {
      ...
      res.json({
        success: true,
        message: "Sign-in successful",
        redirectTo: `/seller-dashboard/${userInfo.user_id}`,
      });
    }
  } catch (error) {
...
router.get("/popular", async (req, res) => {
  try {
    const products = await pool.query(
      "SELECT * FROM product ORDER BY purchase_count DESC LIMIT 12"
    );
    ...
    const productsWithPhotos = await Promise.all(
      products.rows.map(async (product) => {
        try {
          ...
          console.log(
            `Unsplash API URL: https://api.unsplash.com/photos/?query=${product.name}&client_id=GjsMBVfSCqxwVL05d9vPtatujrcKnHhxgyu0oUy5I8Y`
          );
          ...
          await pool.query(
            "UPDATE product SET photo_url = $1 WHERE product_id = $2",
            [randomPhotoUrl, product.product_id]
          );
        } catch (error) {
          console.error(
            `Error fetching or updating for product ${product.product_id}: ${error.message}`
          );
        }
      })
    );
